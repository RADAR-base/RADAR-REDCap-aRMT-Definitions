  ...
  try {
    ...
    github.repos.getContent(
      ...
      function(status, data) {
        console.log(
          'Status: ' + status + ', DATA: ' + data + ', Path: ',
          github_details.path,
        );
        if (data !== undefined) {
          console.log(
            'Updating existing file on Github: ' +
              filename +
              ' on Branch: ' +
              github_details.branch,
          );
...
function postRADARJSON(
  ...
  request.post({ url: redcap_url, form: post_form }, function(
    err,
    httpResponse,
    body,
  ) {
    ...
    setInterval(function() {
      ...
      if (form_armt[0].field_name == 'record_id') form_armt.splice(0, 1);

      switch (type) {
        case 'redcap':
          preparePostToGithub(form_name, form_redcap, lang);

        default:
          preparePostToGithub(form_name, form_armt, lang);
          break;
      }
      globalItter += 1;
    }, 3000);
